...
export const create = (req, res) => {
  ...
  if (!category) return res.status(400).json({ error: 'Vui lòng chọn loại chi phí!' });
  if (!description?.trim()) return res.status(400).json({ error: 'Vui lòng nhập mô tả chi phí!' });
  if (amount === undefined || Number(amount) <= 0) return res.status(400).json({ error: 'Số tiền phải lớn hơn 0!' });
  if (!date) return res.status(400).json({ error: 'Vui lòng chọn ngày!' });
  ExpenseModel.create({ category, description: description.trim(), amount, date, note, locationId }, (err, result) => {
    if (err) return res.status(400).json({ error: err.message || 'Lỗi thêm chi phí!' });
    res.status(201).json({ message: 'Thêm chi phí thành công!', id: result.id });
  });
};
...
export const update = (req, res) => {
  ...
  if (amount !== undefined) { if (Number(amount) <= 0) return res.status(400).json({ error: 'Số tiền phải lớn hơn 0!' }); data.amount = amount; }
