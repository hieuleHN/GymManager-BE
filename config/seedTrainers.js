...
async function seedTrainers() {
  try {
    ...
    for (const t of trainers) {
      ...
      console.log(`Đã tạo HLV: ${t.fullName} (${t.job})`);
    }

    await mongoose.disconnect();
    console.log('\nSeed hoàn tất! Restart server backend rồi refresh trang.');
    process.exit(0);
  } catch (err) {
